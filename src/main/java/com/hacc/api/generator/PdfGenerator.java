...
public class PdfGenerator {
    ...
    public static byte[] buildFinancialReport(List<Pago> pagos) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            // --- TABLA DE DETALLE ---
            Table tabla = new Table(UnitValue.createPercentArray(new float[]{3, 4, 2}))
                    .setWidth(UnitValue.createPercentValue(100))
                    .setMarginBottom(20);

            // Encabezados de tabla
            String[] headers = {"FECHA", "CONCEPTO", "MONTO"};
            ...
            for (Pago p : pagos) {
                ...
                // Concepto
                tabla.addCell(new Cell()
                        .add(new Paragraph(p.getTitulo() != null ? p.getTitulo() : "Sin concepto")
                                .setFont(regularFont)
                                .setFontSize(10))
                        .setPadding(8)
                        .setBorder(new SolidBorder(COLOR_BORDE, 1)));
                ...
                // Monto
                tabla.addCell(new Cell()
                        .add(new Paragraph("$ " + String.format("%,.2f", p.getMonto()))
                                .setFont(regularFont)
                                .setFontSize(10)
                                .setTextAlignment(TextAlignment.RIGHT))
                        .setPadding(8)
            ...
            // --- PIE DE PÁGINA ---
            Paragraph pie = new Paragraph("Reporte generado automáticamente")
                    .setFont(regularFont)
                    .setFontSize(8)
                    .setFontColor(ColorConstants.GRAY)
                    .setTextAlignment(TextAlignment.CENTER)
